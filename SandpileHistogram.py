...
x_fit = np.linspace(np.log(min(bin_centers[mask]))/np.log(podstawa), np.log(max(bin_centers[mask]))/np.log(podstawa), 100)
y_fit = slope * x_fit + intercept

print(slope)
# Rysowanie dopasowanej prostej na istniejącym wykresie
plt.plot(podstawa**x_fit, podstawa**y_fit, color='red', label=f'log-log fit: y ∝ x^{slope:.2f}')
plt.legend()
plt.xscale('log', base=podstawa)
plt.yscale('log', base=podstawa)
